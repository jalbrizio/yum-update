***REMOVED***
***REMOVED*** register a RHEL5 or 6 client to the RHN Satellite server in the traditional manner
***REMOVED***
***REMOVED*** this will overwrite the existing /etc/sysconfig/rhn/up2date, so...
cp -p /etc/sysconfig/rhn/up2date /etc/sysconfig/rhn/up2date-last$$
***REMOVED***
***REMOVED*** REQUIRED: install the satellite SSL cert directly from the Satellite server...
...
***REMOVED***wget http://ncc-1701-sw/pub/RPM-GPG-KEY-MariaDB -O /etc/pki/rpm-gpg/RPM-GPG-KEY-MariaDB
***REMOVED***rpm --import /etc/pki/rpm-gpg/RPM-GPG-KEY-MariaDB
...
***REMOVED*** edit the Red Hat target in /etc/sysconfig/rhn/up2date to point to
***REMOVED*** the Satellite URL, then run  rhn_register  as usual.
...
***REMOVED*** clean any existing yum configuration information...
yum clean all

***REMOVED*** register the client using rhn_register and a userid and passwd...
rhn_register

***REMOVED*** all done - run  yum update  whenever ready...
